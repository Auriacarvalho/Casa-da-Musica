...
app.post('/add', (req, res) => {
  const { nome, codigo, professor_id } = req.body;
  const sql = `INSERT INTO disciplinas (nome, codigo, professor_id) VALUES (?, ?, ?)`;
  db.query(sql, [nome, codigo, professor_id], (err, result) => {
    if (err) throw err;
    res.redirect('/');
  });
});
...
app.post('/update/:iddisciplina', (req, res) => {
  ...
  const sql = `UPDATE disciplinas SET nome = ?, codigo = ?, professor_id = ? WHERE iddisciplina = ?`;
  db.query(sql, [nome, codigo, professor_id, iddisciplina], (err, result) => {
    if (err) throw err;
    res.redirect('/');
  });
});
