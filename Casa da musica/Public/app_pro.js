...
app.post('/add', (req, res) => {
  ...
  db.query(sql, [nome, cpf_cnpj, telefone, email, data_nasc, idade, sexo, cor, cep, endereco, numero, bairro, cidade, estado, complemento, obs], (err, result) => {
    if (err) throw err;
    res.redirect('/');
  });
});
...
app.post('/update/:idprofessor', (req, res) => {
  ...
  const sql = `UPDATE professores SET nome = ?, cpf_cnpj = ?, telefone = ?, email = ?, data_nasc = ?, idade = ?, sexo = ?, cor = ?, cep = ?, endereco = ?, numero = ?, bairro = ?, cidade = ?, estado = ?, complemento = ?, obs = ? WHERE idprofessor = ?`;
  db.query(sql, [nome, cpf_cnpj, telefone, email, data_nasc, idade, sexo, cor, cep, endereco, numero, bairro, cidade, estado, complemento, obs, idprofessor], (err, result) => {
    if (err) throw err;
    res.redirect('/');
  });
});
